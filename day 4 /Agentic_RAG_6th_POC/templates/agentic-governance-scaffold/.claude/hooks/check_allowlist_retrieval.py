#!/usr/bin/env python3
"""PreToolUse: gate any retrieval-shaped tool call against the approved source allowlist for this project.
...
def main():
    ...
    if not host:
        ask(f"Could not parse a hostname from '{url}'. Confirm this call retrieves "
            f"only from an approved source before proceeding.")
        sys.exit(0)
    ...
    try:
        ...
        ask(f"Allowlist file not found at {allowlist_path}. Confirm '{host}' is an "
            f"approved source before proceeding.")
        sys.exit(0)

    allowed_hosts = {h for hosts in allowlist.values() for h in hosts}

    if host not in allowed_hosts:
        ask(f"'{host}' is not on the approved approved source allowlist "
