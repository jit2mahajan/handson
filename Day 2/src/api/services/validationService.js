...
export async function validateRecord(record) {
    try {
        if (import.meta.env.VITE_MOCK_DATA === 'true') {
            ...
            if (record.description?.length > 5000)
                errors.push('Description exceeds max length (5000)');
            // ALCOA+ validation
 ...
 */
export async function validateALCOACompliance(record) {
    try {
        if (import.meta.env.VITE_MOCK_DATA === 'true') {
            ...
            return {
                ...
                overallCompliance: !!record.createdBy &&
                    !!record.createdAt &&
                    !!record.title &&
                    !!record.description &&
                    isRecent,
 ...
 */
export async function checkApprovalEligibility(record) {
    try {
        if (import.meta.env.VITE_MOCK_DATA === 'true') {
            await new Promise(resolve => setTimeout(resolve, 200));
            const reasons = [];
            ...
            if (!record.alcoa.attributable)
                reasons.push('Not attributable');
            if (!record.alcoa.legible)
                reasons.push('Not legible');
            if (!record.alcoa.contemporaneous)
                reasons.push('Not contemporaneous');
            if (!record.alcoa.original)
                reasons.push('Original not preserved');
            if (!record.alcoa.accurate)
                reasons.push('Data not accurate');
            if (!record.alcoa.auditable)
                reasons.push('Not auditable');
            // Check record status
            if (record.status === 'approved')
                recommendations.push('Record already approved');
            if (record.status === 'rejected')
                recommendations.push('Record is rejected');
...
// SERVICE CLEANUP
// ============================================================================
export function cleanupValidationService() {
