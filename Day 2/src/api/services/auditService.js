...
export async function fetchAuditTrail(filters) {
    try {
        ...
        if (filters?.recordId)
            queryParams.append('recordId', filters.recordId);
        if (filters?.action)
            queryParams.append('action', filters.action);
        if (filters?.changedBy)
            queryParams.append('changedBy', filters.changedBy);
 ...
 */
export async function fetchRecordAudit(recordId) {
    try {
        if (import.meta.env.VITE_MOCK_DATA === 'true') {
 ...
 */
export async function exportAuditTrail(format = 'json') {
    try {
        if (import.meta.env.VITE_MOCK_DATA === 'true') {
...
// SERVICE CLEANUP
// ============================================================================
export function cleanupAuditService() {
