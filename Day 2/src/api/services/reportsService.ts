 ...
 */
export async function generateComplianceReport(): Promise<ReportStats> {
  try {
...
export async function exportReportCSV(
  reportType: 'compliance' | 'activity' = 'compliance',
): Promise<Blob> {
  try {
    if (import.meta.env.VITE_MOCK_DATA === 'true') {
      await new Promise(resolve => setTimeout(resolve, 500));
 ...
 */
export async function generatePDFReport(reportType: 'compliance' | 'activity'): Promise<Blob> {
  try {
    if (import.meta.env.VITE_MOCK_DATA === 'true') {
...
export function cleanupReportsService(): void {
