...
import { general, create, loginRateLimit, recordSuccessfulLogin, recordFailedLoginAttempt, getRateLimitStatus, resetRateLimit, getAllRateLimitKeys, } from '../middleware/rateLimit';
// Mock Redis client
...
describe('Rate Limiting Middleware', () => {
    ...
    describe('General Rate Limiting', () => {
        it('should allow requests below limit', async () => {
            ...
            await general(mockReq, mockRes, mockNext);
    ...
    describe('Login Rate Limiting with Exponential Backoff', () => {
        ...
        it('should allow login attempts below limit', async () => {
            ...
            await loginRateLimit(mockReq, mockRes, mockNext);
            expect(mockNext).toHaveBeenCalled();
            expect(mockRes.status).not.toHaveBeenCalled();
        });
    ...
    describe('Per-User Rate Limiting', () => {
        ...
        it('should rate limit per authenticated user for creation', async () => {
            ...
            mockReq.method = 'POST';
            redisClient.incr.mockResolvedValue(51);
            redisClient.ttl.mockResolvedValue(30);
    ...
    describe('Admin Endpoints', () => {
        describe('getRateLimitStatus', () => {
            it('should return current status for a key', async () => {
                ...
                redisClient.ttl.mockResolvedValue(30);
                await getRateLimitStatus(mockReq, mockRes);
                expect(mockRes.json).toHaveBeenCalledWith(expect.objectContaining({
                    key: 'test-key',
                    current: 42,
                    ttl: 30,
        ...
        describe('getAllRateLimitKeys', () => {
            it('should list all active rate limit keys', async () => {
                ...
                redisClient.getKeys.mockResolvedValue(keys);
                redisClient.get
                ...
                await getAllRateLimitKeys(mockReq, mockRes);
                expect(mockRes.json).toHaveBeenCalledWith(expect.objectContaining({
                    count: 2,
    ...
    describe('Edge Cases', () => {
        it('should handle exactly at limit', async () => {
            redisClient.incr.mockResolvedValue(100);
            redisClient.ttl.mockResolvedValue(30);
            await general(mockReq, mockRes, mockNext);
            expect(mockNext).toHaveBeenCalled();
        });
        ...
        it('should handle negative remaining count', async () => {
            redisClient.incr.mockResolvedValue(150);
            redisClient.ttl.mockResolvedValue(30);
            await general(mockReq, mockRes, mockNext);
