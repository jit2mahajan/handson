...
router.get('/status', async (_req: Request, res: Response) => {
  try {
    ...
    res.json({
      ...
      memory_store:
        memoryInfo && health.status === 'memory-fallback'
          ? {
 ...
 */
router.post('/reset-pattern', async (req: Request, res: Response) => {
  try {
    ...
    console.error(
      '[RateLimitStats] Error resetting rate limits by pattern:',
      error
    );
    res.status(500).json({
 ...
 */
router.post('/clear-login/:email', async (req: Request, res: Response) => {
  try {
    const { email } = req.params;
 ...
 */
router.post('/simulate-attempt', async (req: Request, res: Response) => {
  try {
