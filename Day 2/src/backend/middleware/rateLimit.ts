...
import {
  ...
  createRecordRateLimitConfig,
  reportRateLimitConfig,
  auditExportRateLimitConfig,
  adminRateLimitConfig,
  approvalRateLimitConfig,
  redisStore,
...
// Track login attempts for exponential backoff
 ...
 */
function createRateLimitMiddleware(config: typeof generalRateLimitConfig) {
  ...
  return (async () => {
    try {
      ...
      if (attempts.count > 10) {
        ...
        console.warn(
          `[RateLimit] Login blocked due to too many failed attempts`,
          {
            username,
            ip,
      ...
      (req as any).loginAttempts = attempts.count;
 ...
 */
export async function getAllRateLimitKeys(
  _req: Request,
  res: Response
): Promise<void> {
  try {
