...
class RedisClientManager extends EventEmitter {
    constructor() {
        ...
        Object.defineProperty(this, "isUsingMemory", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        });
        Object.defineProperty(this, "connectionRetries", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
    ...
    async initialize(config) {
        try {
            console.log(`[RedisClient] Attempting to connect to Redis at ${config.host}:${config.port}`);
            ...
            console.warn('[RedisClient] Failed to connect to Redis:', error instanceof Error ? error.message : String(error));
    ...
    handleRedisError() {
        if (this.connectionRetries < this.maxRetries) {
            ...
            console.log(`[RedisClient] Retrying connection (${this.connectionRetries}/${this.maxRetries})...`);
            setTimeout(() => {
    ...
    handleConnectionFailure() {
        console.warn('[RedisClient] Redis connection failed, falling back to memory store');
    ...
    memorySet(key, value, expirySeconds) {
    ...
    memoryClearPattern(pattern) {
        const regex = new RegExp('^' + pattern.replace(/\*/g, '.*') + '$');
        const keysToDelete = Object.keys(this.memoryStore).filter((key) => regex.test(key));
    ...
    getMemoryStoreInfo() {
        ...
        const estimatedMemory = `${((JSON.stringify(this.memoryStore).length / 1024).toFixed(2))} KB`;
        return {
            keyCount,
            estimatedMemory,
